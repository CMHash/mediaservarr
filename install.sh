#!/bin/bash
sudo curl http://downloads.plex.tv/plex-keys/PlexSign.key | sudo apt-key add -
...
echo "deb http://apt.sonarr.tv/ubuntu focal main" | sudo tee /etc/apt/sources.list.d/sonarr.list
...
# Create User / Group as needed
if ! getent group "$app_guid" && "$app_guid" != "$app_uid" >/dev/null; then
    groupadd "$app_guid"
    echo "Group [$app_guid] created"
fi
if ! getent passwd "$app_uid" >/dev/null; then
